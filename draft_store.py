...
from report_generator import Issue, month_year_from_date
...
class DraftStore:
    ...
    def save_session(self, session: Session, status: str | None = None) -> None:
        ...
        now = _now_iso()
        data = session.data
        summary_date, summary_project, summary_sub_project = _summary_fields(data)
...
def _summary_fields(data: dict[str, str]) -> tuple[str, str, str]:
    if "date_start" in data or "date_end" in data or "location" in data:
        start = data.get("date_start", "").strip()
        end = data.get("date_end", "").strip()
        if start and end:
            summary_date = f"{start} - {end}"
        else:
            summary_date = start or end
        month_year = month_year_from_date(start) if start else ""
        return summary_date, data.get("location", ""), month_year
    return data.get("date", ""), data.get("project_name", ""), data.get("project_sub_name", "")
