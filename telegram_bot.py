...
from report_generator import ReportData, month_year_from_date, render_report
...
ENV_PATH = Path(".env")
TEMPLATE_PATH = Path("template-weekly-report.odt")
...
def _handle_update(
    ...
    if text == "/start":
        ...
        client.send_message(chat_id, f"Laporan mingguan baharu dimulakan.\n\n{_field_prompt(0)}")
        return
...
def _handle_callback_query(
    ...
    if action == "add_issue":
        ...
        store.save_session(session)
        _set_review_message(client, store, session, "Hantar butiran kerja baharu.", _back_to_review_keyboard())
        return
    ...
    if action == "edit_issue_description" and isinstance(value, int):
        ...
        _set_review_message(
            ...
            store,
            session,
            f"Masukkan butiran baharu untuk kerja {value + 1}.\nSemasa: {issue.description}",
            _back_to_review_keyboard(),
        )
        return
    ...
    if action == "edit_issue_images_description" and isinstance(value, int):
        ...
        _set_review_message(
            ...
            store,
            session,
            f"Masukkan catatan baharu untuk kerja {value + 1}. Balas /skip untuk kosongkan.\nSemasa: {current_value}",
            _back_to_review_keyboard(),
        )
        return
    ...
    if action == "edit_issue_add_image" and isinstance(value, int):
        ...
        _set_review_message(
            ...
            store,
            session,
            f"Hantar gambar baharu untuk kerja {value + 1}. Balas /done apabila selesai.",
            _back_to_review_keyboard(),
        )
        return
    ...
    if action == "select_delete_issue" and isinstance(value, int):
        client.answer_callback_query(callback_id)
        if value >= len(session.issues):
            _show_review(client, store, session, prefix=f"Kerja {value + 1} tidak wujud.\n\n")
            return
        ...
        _set_review_message(
            ...
            store,
            session,
            f'Anda pasti mahu padam kerja {value + 1}?\n"{session.issues[value].description}"',
    ...
    if action == "confirm_delete_issue" and isinstance(value, int):
        ...
        if session.delete_issue_index != value or value >= len(session.issues):
            ...
            session.delete_issue_index = None
            store.save_session(session)
            _show_review(client, store, session, prefix="Kerja itu tidak lagi tersedia untuk dipadam.\n\n")
            return
        removed = session.issues.pop(value)
        session.stage = "review"
        session.delete_issue_index = None
        store.save_session(session)
        _show_review(client, store, session, prefix=f'Kerja {value + 1} dibuang: "{removed.description}"\n\n')
        return
    ...
    if action == "remove_issue_image" and isinstance(value, tuple) and len(value) == 2:
        ...
        if issue_index >= len(session.issues):
            _show_review(client, store, session, prefix=f"Kerja {issue_index + 1} tidak wujud.\n\n")
            return
        ...
        if issue.image_paths:
            ...
            _show_issue_edit_menu(
                ...
                session,
                issue_index,
                prefix=f"Gambar {removed_path.name} telah dipadam. Tiada gambar lagi untuk kerja ini.\n\n",
            )
        return
...
def _finish_report(client: TelegramBotClient, nextcloud: NextcloudClient, store: DraftStore, session: Session) -> int:
    report = ReportData(
        date_start=session.data["date_start"],
        date_end=session.data["date_end"],
        location=session.data["location"],
        issues=session.issues,
    )

    document_path, pdf_path = _build_output_paths(session.workspace, report)
    render_report(TEMPLATE_PATH, document_path, report)
    _convert_document_to_pdf(document_path, pdf_path)
    ...
    _delete_transient_outputs(document_path, pdf_path)
...
def _show_drafts(client: TelegramBotClient, store: DraftStore, chat_id: int) -> None:
    ...
    if not drafts:
        client.send_message(chat_id, "Tiada laporan mingguan aktif. Hantar /start untuk mula baharu.")
        return
...
def _show_issue_selection_menu(
    ...
    if not session.issues:
        _show_review(client, store, session, prefix="Tiada kerja untuk dipilih.\n\n")
        return
    title = "Pilih kerja yang mahu diubah:" if mode == "edit" else "Pilih kerja yang mahu dibuang:"
...
def _show_issue_edit_menu(
    ...
    _set_review_message(
        ...
        store,
        session,
        f"{prefix}Pilih bahagian yang mahu diubah untuk kerja {issue_index + 1}:",
...
def _show_issue_image_selection_menu(
    ...
    _set_review_message(
        ...
        store,
        session,
        f"{prefix}Pilih gambar yang mahu dipadam daripada kerja {issue_index + 1}:",
...
def _build_output_name(report: ReportData, extension: str) -> str:
    date_range = sanitize_filename_part(f"{report.date_start}-{report.date_end}".replace("/", "-"))
    location = sanitize_filename_part(report.location)
    month_year = sanitize_filename_part(month_year_from_date(report.date_start))
    return f"weekly-report-{date_range}-{location}-{month_year}.{extension}"


def _build_output_paths(workspace: Path, report: ReportData) -> tuple[Path, Path]:
    document_path = workspace / _build_output_name(report, "odt")
    pdf_path = workspace / _build_output_name(report, "pdf")
    return document_path, pdf_path


def _convert_document_to_pdf(document_path: Path, pdf_path: Path) -> None:
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(f"Failed to convert document to PDF: {stderr}")
    generated_pdf = document_path.with_suffix(".pdf")
