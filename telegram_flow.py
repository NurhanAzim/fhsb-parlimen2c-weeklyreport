...
def _handle_field_input(client: Any, store: DraftStore, session: Session, text: str) -> None:
    ...
    store.save_session(session)
    client.send_message(session.chat_id, "Hantar butiran kerja pertama. Jika tiada kerja, balas /done.")
...
def _handle_issue_description(
    ...
    if text == "/done":
        ...
        return
    if not text:
        client.send_message(session.chat_id, "Butiran kerja tidak boleh kosong.")
        return
    if len(session.issues) >= max_issues_per_report:
        _enter_review(client, store, session, hooks)
        client.send_message(
            session.chat_id,
            f"Had kerja per laporan telah dicapai ({max_issues_per_report}). Teruskan ke semakan.",
        )
        return
    ...
    client.send_message(
        session.chat_id,
        "Masukkan catatan kerja jika perlu. Jika tiada, balas /skip.",
    )
...
def _handle_issue_images_description(client: Any, store: DraftStore, session: Session, text: str) -> None:
    ...
    store.save_session(session)
    client.send_message(session.chat_id, "Hantar gambar kerja satu demi satu. Bila selesai, balas /done.")
...
def _handle_issue_images(
    ...
    if len(session.current_issue.image_paths) >= max_images_per_issue:
        client.send_message(
            session.chat_id,
            f"Had gambar bagi satu kerja telah dicapai ({max_images_per_issue}). Balas /done untuk teruskan.",
        )
        return
...
def _handle_more_issues(
    ...
    if normalized in {YES_LABEL.lower(), "y", "yes"}:
        ...
        client.send_message(
            session.chat_id,
            "Hantar butiran kerja seterusnya.",
...
def _handle_edit_field(
    ...
    if field_key in {"date_start", "date_end"} and not _is_valid_date(normalized):
        client.send_message(session.chat_id, f"Tarikh tidak sah. {FIELD_GUIDANCE[field_key]}")
        return
    session.data[field_key] = normalized
...
def _handle_edit_issue_description(
    ...
    session.stage = "review"
    store.save_session(session)
    hooks.show_review(client, store, session, prefix=f"Butiran kerja {issue_number} telah dikemas kini.\n\n")
...
def _handle_edit_issue_images_description(
    ...
    if normalized.lower() == "/skip":
        ...
        client.send_message(session.chat_id, "Catatan kerja tidak boleh kosong. Balas /skip untuk kosongkan.")
        return
    ...
    session.stage = "review"
    store.save_session(session)
    hooks.show_review(client, store, session, prefix=f"Catatan kerja {issue_number} telah dikemas kini.\n\n")
...
def _handle_edit_issue_add_images(
    ...
    if text == "/done":
        ...
        session.stage = "review"
        store.save_session(session)
        hooks.show_review(client, store, session, prefix=f"Kemaskini gambar kerja {issue_number} selesai.\n\n")
        return
    ...
    if len(issue.image_paths) >= max_images_per_issue:
        client.send_message(
            session.chat_id,
            f"Had gambar bagi satu kerja telah dicapai ({max_images_per_issue}). Balas /done untuk kembali.",
        )
        return
    if _count_total_images(session) >= max_total_images_per_report:
        client.send_message(
